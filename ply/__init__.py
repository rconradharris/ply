...
def save(since, quiet=False, reset=True):
    ...
    """
    working_repo = git.Repo('sandbox/working-repo')
    patch_repo = git.Repo('sandbox/patch-repo')

    # Create patch files
    orig_filenames = working_repo.format_patch(since)
    ...
    with open(os.path.join(patch_repo.path, 'series'), 'a') as f:
        for orig_filename in orig_filenames:
            orig_path = os.path.join(working_repo.path, orig_filename)
            ...
            new_path = os.path.join(patch_repo.path, new_filename)
    ...
    for filename in new_filenames:
        patch_repo.add(filename)

    patch_repo.add('series')
    ...
    patch_repo.commit('Adding patches', quiet=quiet)

    # Rollback patches from working-repo
    if reset:
        working_repo.reset(since, hard=True, quiet=quiet)
